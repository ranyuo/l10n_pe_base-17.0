...
class LibroReclamaciones(http.Controller):
    ...
    def LibroReclamacionesEnviada(self, code, **kw):
        ...
        return request.render('l10n_pe_libro_reclamaciones.reclamacion_enviada', {"code": code, "post_info": post_info})
        # if request.env.user._is_public():
        #     return request.render('l10n_pe_libro_reclamaciones.reclamacion_enviada')
        # else:
        #     partner_id = request.env.user.partner_id.id
        #     return request.render('l10n_pe_libro_reclamaciones.reclamacion_enviada', {"partner": partner_id})
    ...
    def LibroReclamaciones(self, **args):
        ...
        state_ids = request.env['res.country.state'].sudo().search([('country_id', '=', country_id)])
        ...
        if not bool(args):
            ...
            return request.render('l10n_pe_libro_reclamaciones.libro_reclamaciones', {'states': state_ids,
                                                                                 'company': company,
                                                                                 'errors': errors,
                                                                                 'claim': claim})
        else:
    ...
    def GetDisLibroReclamaciones(self, provincia, **kw):
        return http.request.env['l10n_pe.res.city.district'].sudo().search([('city_id', '=', int(provincia))]).mapped(lambda r: {'id': r.id, 'name': r.name})
    ...
    def GetProLibroReclamaciones(self, departamento, **kw):
        return http.request.env['res.city'].sudo().search([('state_id', '=', int(departamento))]).mapped(lambda r: {'id': r.id, 'name': r.name})
